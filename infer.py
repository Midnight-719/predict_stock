from Model import lstm,RNN,GRU,ResidualLSTM5,ResidualLSTM
...
model.to(device)
checkpoint = torch.load('LSTM-200-1L-50E-10DAY.pth')
model.load_state_dict(checkpoint['state_dict']) # model.load_state_dict(torch.load('LSTM-200-1L-50E-10DAY.pth'))
# model.load_state_dict(checkpoint)
preds = []
labels = []
file = "pp500.csv"

train_dataset_regression, test_dataset_regression, train_loader, test_loader,min_val,max_val = getdata(file,seq_len,batch_size,time_pridict)
model.eval()
for idx, (val, label) in enumerate(test_loader):
    # val--> data
    val = torch.tensor(val, dtype=torch.float32).cuda() #  torch.Size([32, 15, 5])
    val = val.squeeze(1)
    pred = model(val)
...
# max_val: array([4.804510e+03, 4.796560e+03, 4.818620e+03, 4.780040e+03, 5.264098e+06])
# min_val: array([  1833.4    ,   1829.08   ,       1847. ,   1810.1    ,    240297.  ])
# original_preds = (preds * (max_val[1] - min_val[1])) + min_val[1]
# original_labels = (labels* (max_val[1] - min_val[1])) + min_val[1]
original_preds   = []
original_labels  = []
YYY = []
